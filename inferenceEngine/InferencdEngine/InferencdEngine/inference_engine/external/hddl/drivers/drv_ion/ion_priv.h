...
struct ion_buffer {
	...
	char alloc_caller[MAX_FUNC_NAME_LEN];
	int alloc_linenum;
};
...
struct ion_heap_ops {
	...
	int (*phys_and_cpu)(struct ion_heap *heap, struct ion_buffer *buffer,
...
/**
 * ion_get_device - return linux data struct device, it is the misc device
 * @dev:		the device the pages will be used with
 * returns  the struct device
 */
